...
#'
#' @param path File path to the v2rar folder, or full path to the data
...
ss_read_vr2ar_data <- function(path, file_name) {
...
#'
#' @description Compiles and formats temperature and depth data from VR2AR
#'   sensors exported from Fathom software.
#'
#' @details The raw VR2AR data must be saved in a folder named vr2ar in csv
#'   format. Folder name is not case-sensitive.
#'
#' @inheritParams ss_compile_hobo_data
#'
#' @param path File path to the vr2ar folder. This folder should have one csv
#'   file that was extracted using Fathom software. Other file types in the
#'   folder will be ignored.
#'
#' @return Returns a tibble with the data compiled from the file in path/vr2ar.
#'
#' @family compile
#' @author Danielle Dempsey
#'
#' @importFrom dplyr %>% case_when filter group_by mutate select
#'   summarise tibble
...
ss_compile_vr2ar_data <- function(
  ...
  for (i in seq_along(dat_files)) {
    ...
    # Extract metadata --------------------------------------------------------
    dat_i <- ss_read_vr2ar_data(file_i)
    ...
    dat_i <- dat_i %>%
      slice(-c(1:2)) %>%
      select(
        timestamp_utc_uncorrected = `Device Time (UTC)`,
      ...
      mutate(
        timestamp_utc = as_datetime(paste0(timestamp_utc_uncorrected, ":00")),
    ...
    message(paste("vr2ar data from sensor << ", sn_i, " >> compiled"))
  }
